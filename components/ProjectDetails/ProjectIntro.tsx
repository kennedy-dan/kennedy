...
export default function ProjectIntro({ details }: ProjectIntroProps): React.ReactElement {
    ...
    <article className="space-y-20">
      ...
      <div className="w-full h-[60vh] lg:h-screen bg-gray-50 ">
        <img
          src={appImage}
          alt={name}
          className={`${_id === "project-2" ? "object-contain" : "object-cover"} w-full h-full  md:grayscale hover:grayscale-0 transition-all duration-1000`}
        />
      </div>
  ...
  <span className="w-6 h-[1px] bg-gray-300" />
        ...
        {/* Thumbnail */}
        <div className="aspect-[18/9] mb-10 bg-gray-50 border border-gray-100 overflow-hidde">
          <img
            src={thumbnail}
            alt={`${name} overview`}
            className={` ${_id === "project-2" ? "object-contain" : "object-cover"} w-full h-full`}
          />
        </div>
      </div>
    </article>
